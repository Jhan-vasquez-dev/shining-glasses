...
public class Analyzer {
    ...
    private static int getMaxDimensionTraversal(ConstraintWidget constraintWidget, int i, boolean z, int i2) {
        ...
        while (it.hasNext()) {
            iMax2 = Math.max(iMax2, getMaxDimensionTraversal(((ResolutionAnchor) it.next()).myAnchor.mOwner, i, z, i9));
        }
        Iterator<ResolutionNode> it2 = constraintWidget.mListAnchors[i3].getResolutionNode().dependents.iterator();
        int iMax3 = 0;
        while (it2.hasNext()) {
            iMax3 = Math.max(iMax3, getMaxDimensionTraversal(((ResolutionAnchor) it2.next()).myAnchor.mOwner, i, z, width2 + i9));
            z2 = z2;
        }
        boolean z3 = z2;
        if (z3) {
            iMax2 -= height;
            width = iMax3 + baselineDistance;
        } else {
            width = iMax3 + ((i == 0 ? constraintWidget.getWidth() : constraintWidget.getHeight()) * i5);
        }
        int i10 = 1;
        if (i == 1) {
            ...
            while (it3.hasNext()) {
                ...
                if (i5 == i10) {
                    iMax = Math.max(i11, getMaxDimensionTraversal(resolutionAnchor.myAnchor.mOwner, i, z, height + i9));
                } else {
                    iMax = Math.max(i11, getMaxDimensionTraversal(resolutionAnchor.myAnchor.mOwner, i, z, (baselineDistance * i5) + i9));
                }
                i11 = iMax;
                it3 = it4;
                i10 = 1;
            }
            i6 = (constraintWidget.mBaseline.getResolutionNode().dependents.size() <= 0 || z3) ? i11 : i5 == 1 ? i11 + height : i11 - baselineDistance;
        } else {
            i6 = 0;
        }
        int iMax4 = margin + Math.max(iMax2, Math.max(width, i6));
        int i12 = width2 + i9;
        if (i5 == -1) {
            i12 = i9;
            i9 = i12;
        ...
        if (z) {
            ...
            constraintWidget.setFrame(i9, i12, i);
        } else {
