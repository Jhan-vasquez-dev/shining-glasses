    ...
    */
    ...
    public static final int process3Utf8Bytes(byte[] process3Utf8Bytes, int i, int i2, Function1<? super Integer, Unit> yield) {
        ...
        if (i2 <= i3) {
            yield.invoke(numValueOf);
            int i4 = i + 1;
            return (i2 <= i4 || (process3Utf8Bytes[i4] & JSONB.Constants.BC_INT64_SHORT_MIN) != 128) ? 1 : 2;
        }
    ...
    public static final int process4Utf8Bytes(byte[] process4Utf8Bytes, int i, int i2, Function1<? super Integer, Unit> yield) {
        ...
        if (i2 <= i3) {
            yield.invoke(numValueOf);
            int i4 = i + 1;
            if (i2 <= i4 || (process4Utf8Bytes[i4] & JSONB.Constants.BC_INT64_SHORT_MIN) != 128) {
                return 1;
            }
            int i5 = i + 2;
            return (i2 <= i5 || (process4Utf8Bytes[i5] & JSONB.Constants.BC_INT64_SHORT_MIN) != 128) ? 2 : 3;
        }
